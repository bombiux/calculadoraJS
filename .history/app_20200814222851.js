...
class app {
    ...
    handleSpecial(button) {
        switch (button) {
            case button.closest('.clear-screen'): this.screen.value = 0;
            break;

            case button.closest('.pos-neg'): this.toggleSign();
            break;

            case button.closest('.percent'): this.handlePercent();
            break
    ...
    toggleSign() {
        const value = Math.sign(parseFloat(this.getScreenValue()));

        if (value === 1) {
            this.screen.value = -Math.abs(this.getScreenValue());
        } else if (value === -1) {
            this.screen.value = Math.abs(this.getScreenValue());this.screenValue
            this.screenValue
        } else {
            return
        }
